...
<script>
import AddSoldPlatform from './add'
import EditSoldPlatform from './edit'
import { soldPlatformDelete, soldPlatformPageSearch } from '../../../api/sold-platform'

export default {
  name: 'SoldAccountIndex',
  components: { EditSoldPlatform, AddSoldPlatform },
  data() {
    return {
      totalCount: 0,
      currentPage: 1,
  ...
  methods: {
    ...
    flushPage(pageIndex) {
      ...
      soldPlatformPageSearch(queryParams).then(res => {
    ...
    doRemove(row) {
      this.$confirm(
        ...
        soldPlatformDelete(row.id).then(res => {
          if (res.success) {
